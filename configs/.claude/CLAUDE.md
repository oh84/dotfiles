...
以下の使い分けを厳守すること（via https://x.com/t_wada/status/904916106153828352 ）:

- コードには **How** — どう動くかはコード自体に語らせる
- テストコードには **What** — 何を保証するかを表現する
- コミットログには **Why** — なぜこの変更をするのかを書く
- コードコメントには **Why not** — なぜ別のやり方をしなかったのかを書く

つまり、コードを読めばわかること（How や What）をなぞるだけのコメントは書かない。コメントを書いてよいのは、コードから読み取れない制約や「あえてこうしなかった理由」がある場合だけ。
